    ...
    while true do
        for _, data in pairs(explorationData) do
            if tonumber(data.remainingtime) then
                ...
                if timeLeft > 0 then
                    data.remainingtime = tostring(timeLeft - 1)
...
task.spawn(function()
    ...
    -- Mở giao diện Exploration
    OpenExplorationUI()

    -- Đợi một chút để giao diện được mở
    task.wait(2)

    -- Lấy dữ liệu Exploration
    TakeDataExploration()

    -- Bắt đầu cập nhật thời gian remainingtime
        ...
        while autoDeployEnabled do
            ...
            if #deployQueue > 0 then
                ...
                if success then
                    ...
                    local durationInSeconds = convertDurationToSeconds(data.duration)
